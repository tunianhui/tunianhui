<template>
  <section class="physical-config">
    ...
    <el-form
      ...
      label-suffix="："
    >
      <el-form-item label="存储生命周期(天)" prop="cycle_day" class="custom-item">
        <el-input-number v-model.number="filter.cycle_day" class="fl" @change="cycleDayChange"></el-input-number>
      ...
      <el-form-item label="计算逻辑" prop="exec_engine">
        <el-select v-model="filter.exec_engine" placeholder="请选择计算逻辑" filterable>
          <el-option
      ...
      <el-form-item label="无需物理化字段" prop="no_physics_element">
        ...
        >
          <el-option
            v-for="(field, index) in noPhysicsElementList.filter(item => item.visible)"
            :key="field.value + index"
            :label="field.label"
            :value="field.value"
          ></el-option>
...
export default {
  ...
  methods: {
    processFieldList(data) {
      ...
      data.forEach(item => {
        if (
          item.group_type === 'primarykey' &&
          !data.modified &&
          (!data.physics_element || !data.physics_element.length)
        ) {
          this.filter.physics_element = [item.element_id]
          this.prevPhysicsElement = [item.element_id]
          this.sortList = [
            {
        ...
        if (item.group_type !== 'system') {
          this.curElementList.push({
            label: item.element_code,
            value: item.element_id + '',
            isPrimaryKey: item.group_type === 'primarykey',
            visible: true
          })
        }
        if (['attr', 'measure'].includes(item.group_type)) {
          this.noPhysicsElementList.push({
            label: item.element_code,
            value: item.element_id + '',
            visible: true
          })
