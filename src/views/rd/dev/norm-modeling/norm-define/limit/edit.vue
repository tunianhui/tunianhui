<template>
  <section class="limit-atom-edit" ref="container">
    <div class="edit-content">
      ...
      <el-row :gutter="20" ref="tab" type="flex" class="mt-10 mb-10">
        <el-col :span="14">
          <div class="third-title">业务限定所在来源</div>
          <template v-if="!!editForm.fact_logic_base_id">
            ...
            <div class="sub-title">
              <div class="title">
                <span v-if="activeTab === 'whole'">全局业务限定列表({{tableData.length}})</span>
                <template v-else>
                  <span>
      ...
      <div class="empty-content text-center" v-if="tableData.length===0">
        ...
        <p class="empty-text">{{editForm.fact_logic_base_id ? '暂时还没有任何业务限定文件哦' : '请先选择业务限定来源表哦'}}</p>
        <el-button
      ...
      <el-row :gutter="20" v-else>
        <el-col :span="activeTab === 'whole' ? 24 : 14">
          ...
          >
                ...
                <div class="flex-layout" style="align-items: center;">
                  <div style="max-width: calc(100% - 34px);">
                    <p
                      ...
                      :title="scope.row.condition_alias"
                    >{{scope.row.condition_alias}}</p>
                    <p
                      class="text text-ellipsis"
                      :title="scope.row.condition_name"
                    >{{scope.row.condition_name}}</p>
                  </div>
                  <el-popover
            ...
            <el-table-column prop="condition_desc" label="来源详情" show-overflow-tooltip></el-table-column>
              ...
              <template #default="scope">
                ...
                <el-button
                  ...
                  @click="handleRelatedDerive(scope.row, {
                    key: 'condition_name',
                    value: scope.row.condition_name,
                    id: scope.row.id
                  })"
                ></el-button>
                <el-tooltip popper-class="more-popover">
                  <ul class="popper-list" slot="content">
                    ...
                    <li @click.stop="handleOffLine(scope.row, '业务限定')">
                      <el-button
                        type="text"
  ...
  methods: {
    handleEdit(row, title) {
      ...
      this.setActiveRdEditDialog({
        ...
        data: {...row, fact_logic_base_id, data_field_id, data_field_name}
      })
    },
    handleCreate() {
      const {fact_logic_base_id, data_field_id, data_field_name} = this.editForm
    ...
    handleSameSource(row) {
      this.tabStore.commit('add', {
        label: '同源业务限定 ' + row.condition_name,
        tabType: types.LIMIT,
        name: row.id + '',
        component: 'LimitEdit',
        data: {
    ...
    async getFactList() {
      ...
      const res = await this.$simpleAsyncTo(
        getSourceTableModelInfoFact({
          ...
          data_field_name,
          project_name,
          condition_code: this.activeField.element_code
        }),
        '获取列表数据失败'
      )
      if (res) {
