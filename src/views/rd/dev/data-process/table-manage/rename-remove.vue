<template>
  <el-dialog
    custom-class="rename-remove-edit-dialog"
    :title="title"
        ...
        <el-select v-model="form.catalogid" placeholder="请选择" ref="treeSelect" :disabled="title.indexOf('重命名') > -1" popper-class="select-tree">
          <el-option
            ...
            <el-tree
              ...
              ref="tree" style="font-weight:400">
            </el-tree>
          </el-option>
        </el-select>
      </el-form-item>
...
<script>
import {saveFolder,moveWorkOrderOrFolder,renameFolder} from '@/api/rd/table-manage.js'
      ...
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          if (this.title.indexOf('移动') > -1) {
            ...
            const res = await this.$simpleAsyncTo(moveWorkOrderOrFolder(params), '移动失败')
            if (res) {
              this.$message({
                type: 'success',
                message: '移动成功'
            ...
            const res = await this.$simpleAsyncTo(renameFolder(params), '重命名失败')
            if (res) {
              this.$message({
                type: 'success',
