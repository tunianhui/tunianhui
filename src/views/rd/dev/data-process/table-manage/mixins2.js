...
export default {
  ...
  async created() {
    ...
    if (this.title.indexOf('新建') > -1) {
      if (this.treeData && this.treeData.length) {
        this.table_basic.parent_id = this.treeData[0].id
        this.table_basic.file_catalog = this.treeData[0].name
      }
    }
    if (this.title.indexOf('移动') > -1 || this.title.indexOf('重命名') > -1) {
      let result = this.getCurrentPart(this.editData.parent_id, this.treeData)
      this.table_basic.name = this.editData.name
      this.table_basic.parent_id = result.id
      this.table_basic.file_catalog = result.name
  ...
  methods: {
    ...
    handleNodeClick(data, node) {
      this.table_basic.parent_id = data.id
      this.table_basic.file_catalog = data.name
