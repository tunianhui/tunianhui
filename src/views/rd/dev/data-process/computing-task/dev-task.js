...
export default {
  ...
  methods: {
    ...
    getContextmenu(data) {
      ...
      // 任务类型
      if (data.node_type === '2') {
        menus.push(modifyProcess)
        menus.push(deleteProcess)
    ...
    removeFile (data, node) {
      this.$confirm(`请确认是否删除【${data.label}】？`, '删除', {
        ...
        .then(async () => {
          ...
          if (node_type === '2') {
            const params = {
              id,
              projectId: this.project.project_id // 所属项目id
            }
            const res = await this.$simpleAsyncTo(
          ...
          if (node_type === '1') {
            const params = {
              id, // 节点id
              flowId: 'a7737fac5f8a4ffca1405a608d84a5b4', // 所属业务流程id
              projectId: this.project.project_id // 所属项目id
            }
            // 查询一个id就行 删除应该也可以
            const res = await this.$simpleAsyncTo(
