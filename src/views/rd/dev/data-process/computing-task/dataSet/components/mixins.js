 ...
 * @LastEditTime: 2023-04-24 13:56:55
 * @Description:
 */
import {
  getWorkers,
...
export default {
  ...
  data() {
    return {
      ...
      configData: {
        ...
        timeoutNotifyStrategyList: ['WARN'],
        timeoutFlag: 'CLOSE',
        timeout: 30,
        preTaskList: []
      },
    ...
    nodeData: {
      ...
      handler (val) {
        if (val.name) {
          ...
          this.configForm() // 初始化
          this.form.preTaskList = this.nodeData.preTaskList // 前置任务
          this.form.preList = this.nodeData.preList// 前置条件
          this.form.branchList = this.nodeData.branchList // 分支节点
        }

        this.getWorkFlowsData(this.project.skd_prj_id)
      }
    },
    'form.timeoutNotifyStrategyList': {
      handler (val) {
        this.form.timeoutNotifyStrategy = val.join('')
      }
    },
    'project.skd_prj_id': {
      handler (val) {
        this.getTaskGroupsData(val)
  ...
  methods: {
    ...
    async getDataSourceData () {
      const res = await getDatasourceEntitys()
      if (res) {
