 ...
 * @LastEditTime: 2023-04-21 15:25:26
 * @Description:
-->
<template>
  <div class="node-data-wrapper">
    <el-form ref="form" :model="form" :rules="rules" label-position="top">
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="12">
            <el-form-item label="超时时长（分）" prop="timeout">
              <el-input-number v-model="form.timeout" :min="1" :max="10000"></el-input-number>
            </el-form-item>
          </el-col>
        </el-row>
      </template>
      <el-row :gutter="10">
        ...
        <el-col :span="12">
          <el-form-item label="日志显示（行）" prop="displayRows">
            <el-select v-model="form.displayRows" placeholder="请选择">
      ...
      <el-form-item label="SQL语句" prop="sql">
        <MonacoEditor :code.sync="form.sql" style="height: 300px;" />
      </el-form-item>
      ...
      <el-form-item label="前置SQL语句" prop="preStatements">
        <el-button type="primary" circle icon="el-icon-plus" @click="handleAddPreStatements"></el-button>
      </el-form-item>
      <template v-if="form.preStatements && form.preStatements.length">
        <el-row :gutter="10" v-for="(item,index) in form.preStatements" :key="index" class="mt-10">
          <el-col :span="18">
            <el-form-item label="" :key="item.key">
              <el-input v-model="item.prop" placeholder="请输入非查询SQL语句"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="2">
            <el-button type="danger" icon="el-icon-delete" circle @click="delPre(item,index)"></el-button>
          </el-col>
        </el-row>
      </template>
      <el-form-item label="后置SQL语句" prop="postStatements">
        <el-button type="primary" circle icon="el-icon-plus" @click="handleAddPostStatements"></el-button>
      </el-form-item>
      <template v-if="form.postStatements && form.postStatements.length">
        <el-row :gutter="10" v-for="(item,index) in form.postStatements" :key="index" class="mt-10">
          <el-col :span="18">
            <el-form-item label="" :key="item.key">
              <el-input v-model="item.prop" placeholder="请输入非查询SQL语句"></el-input>
            </el-form-item>
          ...
          <el-col :span="2">
            <el-button type="danger" icon="el-icon-delete" circle @click="delPost(item,index)"></el-button>
          </el-col>
        </el-row>
      </template>
