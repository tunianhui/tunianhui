<template>
  <section class="regular side-view">
    ...
    ></el-tree>
    <div class="sv-bottom" @click="openObjectTable('OBJECT')">
...
import {getListAll, del} from '@/api/rd/exreact/object'
...
export default {
  ...
  methods: {
    ...
    recursionTreeData(source, target) {
      source.forEach(item => {
        const {objectCode, objectName, id} = item
        const treeItem = {
          name: objectName,
          code: objectCode,
          label: objectName || objectCode,
          type: FILE_TYPE,
          // parentId: parentId + '',
