<template>
  <section class="regular side-view">
    <div class="sv-top flex-layout space-between"
         @click="goBehaviorBoard">
      <span>
...
import { getListAll, del } from '@/api/rd/exreact/action'
...
export default {
  ...
  methods: {
    ...
    recursionTreeData (source, target) {
      source.forEach(item => {
        const { actCode, actName, id } = item
        const treeItem = {
          name: actName,
          code: actCode,
          label: actName || actCode,
          type: FILE_TYPE,
          // parentId: parentId + '',
