 ...
 * @LastEditors: 大炸鹅
 * @LastEditTime: 2023-02-09 18:56:59
 * @FilePath: \数据智能构建\src\views\rd\exreact\idsCenter\entity-id\attr.vue
-->
<template>
  <div class="entoty-id-attr">
    <div class="subtitle">
      <div>
        <span class="mr-10">{{data.data.idName}}</span>
        <span class="status-tag-right" v-if="data.data.status">{{getDicsValue('status_tag',  data.data.status)}}</span>
      </div>
      <div class="attr-name">{{data.data.idType}}</div>
    </div>
    ...
    <div class="my-content">
      <div class="my-basc mb-20">
        ...
        <div style="display: flex;align-items: center;">
          <span class="my-title ml-10 mt-10">所属类目：</span>
          <span class="ml-10 mt-10">默认</span>
        </div>
        <div style="display: flex;align-items: center;">
          <span class="my-title ml-10 mt-10">最近更新人：</span>
          <span class="ml-10 mt-10">{{infoObj.operUser}}</span>
        </div>
        <div style="display: flex;align-items: center;">
          <span class="my-title ml-10 mt-10">描述：</span>
          <span class="ml-10 mt-10">{{infoObj.remark}}</span>
        </div>
      </div>
      <div class="my-basc">
...
<script>
import { queryByIdInfo, deleteIdEntity } from '@/api/rd/exreact/entity-id'
  ...
  methods: {
    ...
    delHandle(data) {
      this.$confirm(`请确认是否删除【${data.label}】文件？`, '删除', {
        ...
        .then(async () => {
          const res = await this.$simpleAsyncTo(deleteIdEntity({id: data.id}))
          if (res) {
            this.$message({
              type: 'success',
              message: '删除成功！'
    ...
    async _queryByIdInfo() {
      const res = await this.$simpleAsyncTo(queryByIdInfo({id: this.data.data.id}), '获取数据失败')
      if (res) {
