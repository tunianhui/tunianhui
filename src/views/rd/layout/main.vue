            ...
            <el-tabs ref="tabs" v-model="activeTabName" type="card" closable @tab-remove="removeTab" @tab-click="clickTab">
              <el-tab-pane v-for="tab in tabs" :key="tab.name" :label="tab.label" :name="tab.name" lazy v-show="!visibleWellcome && tab.name === activeTabName">
                <span class="label" slot="label" :type="
                  ...
                  <i :class="getIcon(tab.tabType)" :style="{color: getColor(tab.tabType)}"></i>
                  <OverflowTooltip>
                    <span>{{ tab.label }}</span>
                  </OverflowTooltip>
                </span>
...
const Config = _ => import(/* webpackChunkName: "Config" */ './right-sidebar/config.vue')
const History = _ => import(/* webpackChunkName: "History" */ './right-sidebar/history.vue')
const Blood = _ => import(/* webpackChunkName: "Blood" */ './right-sidebar/blood.vue')
const Assistant = _ => import(/* webpackChunkName: "assistant" */ './right-sidebar/assistant.vue')
const Attribute = _ => import(/* webpackChunkName: "attribute" */ './right-sidebar/attribute.vue')
...
export default {
  ...
  data () {
    ...
    return {
      sidebarData: [
        ...
        { label: '属性', name: 'attribute', component: Attribute },
        { label: '助手', name: 'assistant', component: Assistant },
        { label: '历史', name: 'history', component: History },
        { label: '血缘', name: 'blood', component: Blood },
        { label: '配置', name: 'config', component: Config }
      ],
      splitWidth: width,
  ...
  watch: {
    ...
    activeAttr (val) {
      if (val && val.name && val.label !== 'node') {
