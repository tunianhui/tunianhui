  ...
  <section :class="['msl-aside', collapse && 'msl-aside-collapsed']">
    <div class="msl-aside-main dark-loading">
      <div class="t-opreate">
        ...
        <i class="icon iconfont icon-tianjia" title="新建文件" v-if="visibleQuicklyAdd" v-popover:createPopper></i>
      ...
      <div class="navs">
        <!-- 数据集成 不显示tabs -->
        <!-- 这种处理存在问题 -->
        <ul v-show="side_menu.filter(menu => !menu.meta.hideInMenu).length>1" class="tabs">
          <li v-for="tab in side_menu.filter(menu => !menu.meta.hideInMenu)" :key="tab.name" :class="{'is-active': tab.name === activeThirdMenu}" @click="changeMenu(tab)">
            <touch-ripple>{{ tab.meta.name }}</touch-ripple>
          </li>
        </ul>
        <div class="tab-pane">
    ...
    <el-dialog custom-class="form-dialog" title="选择项目" width="500px" :visible.sync="visibleDialog" :close-on-click-modal="false" :close-on-press-escape="false" @close="selectProject">
      <el-form label-width="100px" ref="projectForm" :model="projectForm" :rules="rules">
...
export default {
  ...
  inject: ['tabStore', 'components', 'editComponents', 'editComponentsMap'], // editComponents
  data () {
    return {
      // activeProject: '',
      activeThirdMenu: '',
