 ...
 * @LastEditTime: 2023-08-07 11:25:32
 * @Description: redis维表
-->
  ...
  <div class="comp-option-convert">
    <el-form ref="form" label-width="100px" :model="form" :rules="rules" label-suffix="：">
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <el-form-item label="业务前缀" prop="bizPrefix">
            <el-input v-model="form.bizPrefix" placeholder="请输入业务前缀" :maxlength="64"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="关联字段" prop="joinColumn">
            <el-select v-model="form.joinColumn" clearable placeholder="请选择关联字段">
              <el-option v-for="(item, index) in dataSourceList" :key="index" :label="item.dataSourceName" :value="item.dataSourceId"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="关联类型" prop="joinType">
            <el-select v-model="form.joinType" clearable placeholder="请选择关联类型">
              <el-option v-for="(item, index) in joinTypeList" :key="index" :label="item.itemValue" :value="item.itemCode"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="值字段名" prop="valueColumn">
            <el-input v-model="form.valueColumn" placeholder="请输入值字段名" :maxlength="64"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
    <el-row :gutter="20" style="height: calc(100% - 50px);">
      <el-col :span="12" class="left-col h100">
        ...
        <el-table :data="form.inputColumn" style="width: 100%; height: 280px; overflow: auto;">
      ...
      <el-col :span="12" class="right-col h100">
        ...
        <el-input v-model="keyword" prefix-icon="el-icon-search" @input="searchChange" clearable placeholder="请输入关键词" class="mb-10"></el-input>
        <el-table row-key="name" ref="singleTable" :data="form.outputColumn" style="width: 100%; height: 280px; overflow: auto; cursor: move;">
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        bizPrefix: [
          { required: true, message: '请输入业务前缀', trigger: 'blur' }
        ],
        valueColumn: [
          { required: true, message: '请输入值类型', trigger: 'blur' }
        ]
      },
      // 是否显示字段管理
      dialogVisible: false,
  ...
  methods: {
    ...
    async configData () {
      const dictCodes = ['REDISLOOKUP_JOINTYPE']
      const results = await Promise.all(dictCodes.map(dictCode => getdictitemlist({ dictCode })))
      this.joinTypeList = results[0].data.map(ele => ele.itemValue)
    },
