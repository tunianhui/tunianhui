  ...
  <div class="zone no-hover">
    <div class="animated fadeInRight">
      <div class="select-time mb-20">
        ...
        <el-date-picker v-model="currentTime" type="date" placeholder="选择日期" :picker-options="pickerOptions" value-format="yyyy-MM-dd" @change="timeChangeHandle">
        </el-date-picker>
      </div>
      <div class="select-field">
        <span class="mr-10">字段</span>
        <el-select v-model="currentField" multiple placeholder="请选择" size="medium" :clearable="true" @change="fieldChangeHandle">
      ...
      <div class="field-date">
        <el-table stripe :data="tableData" v-loading="!!loading">
          <el-table-column prop="column_name" label="字段" width="180"></el-table-column>
          <el-table-column :label="currentTime">
            <template slot-scope="{row}">
              <el-tooltip class="item" effect="dark" placement="right" v-for="(item, index) in row.partitions" :key="index">
                <div slot="content">
                  <span class="mb-20" style="padding:5px 0">分区时间：{{ item.create_time }}</span><br />
                  <!-- <span style="margin-top:20px;">字段：{{row.column_name}}</span><br/> -->
                  <span style="margin-top:20px;">表条数：{{ item.num_rows }}</span><br />
                  <span style="margin-top:20px;">表大小：{{ item.raw_size }}</span><br />
                  <span style="margin-top:20px;">hdfs大小：{{ item.hdfs_size }}</span>
                </div>
