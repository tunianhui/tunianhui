 ...
 * @LastEditTime: 2021-08-20 17:22:43
 * @Description:
-->
<template>
  <section class='edition-tab'>
    <div class="manage-current" ref="editionBox">
        ...
        <el-table-column label="操作" width="120">
          <template slot-scope="scope">
            <el-tooltip class="item" effect="dark" content="废止" placement="top">
              <el-button type="text"><i class="el-icon-delete font-16 color-btn" title="废止" @click="abandoned(scope.row)"></i></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    async getTableList(curFilter = [], keyword = '') {
      ...
      const res = await this.$simpleAsyncTo(getCurrentList(params), '获取数据失败')
      if (res) {
        this.showTip = true
    ...
    async _abandoned(row) {
      let params = {std_id: row.std_id}
      const res = await this.$simpleAsyncTo(abandoned(params), '获取数据失败')
      if (res) {
        this.$message({
          message: res.msg,
          type: 'success'
