<template>
  <Layout ref="layout"
          body-class="msl-body"
          class="mutl-sidemenu-layout"
          :data="!$route.path.includes('/asset/standard/edition') ? sidebarData : []"
          @close="closeSidebar">
    <Split v-model="splitWidth"
      ...
      <SideNav slot="left"
               ...
               :visibleQuicklyAdd="visibleQuicklyAdd"></SideNav>
      <template slot="right"
                class="msl-main"
                ref="main">
        <transition name="fadeinright"
                    mode="out-in">
          <component ref="wellcome"
                     :is="components.Wellcome"
                     v-if="!tabs.length"></component>
          <div class="msl-tabs"
               v-else>
            <div class="tab-controls">
              <i class="iconfont icon-sidebar-toggle-copy"
                 v-popover:tabControlsPopover
                 @click="setWellcomeVisible(false)"></i>
              <el-tooltip content="查看操作指引"
                          placement="top">
                <i :class="['iconfont icon-zhiyin', {active: visibleWellcome}]"
                   @click="setWellcomeVisible(true)"></i>
              </el-tooltip>
              <el-popover ref="tabControlsPopover"
                          ...
                          :width="200"
                          :visible-arrow="false">
                <ul class="popper-list tabs-list"
                    :style="{'max-height': `${tabsListMaxHeight}px`}">
                  <li v-for="tab in tabs"
                    ...
                    <touch-ripple class="space-between">
                      <span>
                        <i :class="getIcon(tab)"
                           :style="{color: getColor(tab)}"></i>
                        &nbsp;
                        <span>{{ tab.label }}</span>
                      </span>
                      <i class="el-icon-close"
                         @click.stop="removeTab(tab.name)"></i>
                    </touch-ripple>
                  </li>
                </ul>
            ...
            <el-tabs ref="tabs"
              ...
              <el-tab-pane v-for="tab in tabs"
                ...
                <span class="label"
                  ...
                  "
                      :title="tab.label">
                  <!-- :ref="['codeTask', 'adSearch'].includes(tab.tabType) && activeTabName === tab.name ? 'label' : ''" -->
                  <i :class="tab.icon"
                     :style="{fontSize: '12px', verticalAlign: 'top'}"></i>
                  <!-- <i
                    :class="getIcon(tab.tabType)"
                    :style="{color: getColor(tab.tabType)}"
                  ></i> -->
                  <OverflowTooltip>
                    <span>{{ tab.label }}</span>
                  </OverflowTooltip>
              ...
              <transition name="fadeinleft"
                ...
                <component ref="wellcome"
                           :is="components.Wellcome"
                           v-if="visibleWellcome"></component>
              </transition>
            </el-tabs>
          </div>
        </transition>
...
const MyFile = _ =>
  import(/* webpackChunkName: "MyFile" */ './right-sidebar/myfile.vue')
const Attr = _ =>
  import(/* webpackChunkName: "Attr" */ './right-sidebar/attr.vue')
const FileQuote = _ =>
  import(/* webpackChunkName: "FileQuote" */ './right-sidebar/file-quote.vue')
...
export default {
  ...
  data () {
    ...
    return {
      sidebarData: [
        { label: '我的文件', name: 'myfile', component: MyFile },
        { label: '属性查看', name: 'attr', component: Attr },
        {
          label: '文件引用',
          name: 'filequote',
          component: FileQuote,
  ...
  watch: {
    ...
    activeAttr (val) {
      val && val.name && this.$refs.layout.setActiveSidebar(this.sidebarData[1])
    },
    tabs: {
      immediate: true,
      handler (val, oldVal) {
