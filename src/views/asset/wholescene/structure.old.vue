    ...
    return {
      ...
      legends: [
        ...
        },
        {
          label: '业务过程',
          data: [
            {label: '业务过程', checked: true, icon: factIcon}
          ]
        },
        {
          label: '关系',
  ...
  methods: {
    ...
    processItemsData(data, position) {
      ...
      return data.reduce((prev, cur, index) => {
        ...
        if (position === 'outter') {
          size = textSize(name)
          const pos = this.getArcSpotPos(center, radius + size.width + circleSize / 2 + 10, angle)
          x = pos.x
          y = pos.y
        }
        prev[id] = {
    ...
    drawPaths() {
      ...
      relations.forEach(item => {
        ...
        if (sourcesMap[target]) {
          const {angle: sangle, rotate: srotate, x: sx, y: sy, textSize: stextSize} = sourceOpt,
            {angle: eangle, rotate: erotate, x: ex, y: ey, textSize: etextSize} = targetOpt,
