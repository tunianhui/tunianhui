...
export default {
  ...
  methods: {
    ...
    async mousemove(e) {
      ...
      if (graphic) {
        this.activeGraphic = this.data.list.find(
    ...
    async drawGraphicsAsync() {
      ...
      for (const d of this.data.list) {
        const material = await this.textureLoader(
          `/t-sources/images/global/${i + 1}.png`
        )
    ...
    drawGraphicsPromise() {
      this.fontLoader(this.fontUrl).then(font => {
        const promiseArr = this.data.list.map((d, i) =>
          this.textureLoader(`/t-sources/images/global/${i + 1}.png`)
        )
    ...
    //   const {value, name, unit} = this.data.list[i],
    ...
    //       })
    //     ),
    //     nameText = this.drawText(name, font, 10),
    //     valueText = this.drawText(value + unit, font, -10),
    ...
    //   group.angle = Math.PI + ((Math.PI * 2) / this.data.list.length) * i
    ...
    drawGraphic(material, font, i) {
      const {value, name, unit} = this.data.list[i],
        ...
        // nameText = this.drawText(name, font, 10),
        // valueText = this.drawText(value + unit, font, -10),
        text = this.drawText(name, value + unit),
        group = new this.THREE.Group()
    ...
    drawText(name, value) {
      ...
      this.$refs.graph.appendChild(canvas)
      return mesh
    },
    ...
    graphicsEnter() {
      ...
      this.graphicsEnterIn =
        this.graphics && this.graphics.length === this.data.list.length
    ...
    updateData() {
      this.graphics = this.graphics.map(g => {
        const gd = this.data.list.find(d => d.name === g.name)
        if (!gd) return
        const {value, unit} = gd,
