import VThree from '@/libs/visual/v-three/index.js'
// import { MTLLoader, OBJLoader } from 'three-obj-mtl-loader'
import { MTLLoader } from 'three/examples/jsm/loaders/MTLLoader'
import { OBJLoader } from 'three/examples/jsm/loaders/OBJLoader'

export default class GlobalChart extends VThree {
  ...
  beforeTrender() {
    ...
    this.registerMouseEvent('mousemove', this.mousemove, true)
    this.addHelper()
  ...
  drawTracks() {
    ...
    Array.from({ length: 30 }, (t, i) => {
      if (i === 1) return
      const track = this.drawTrack(size * i, -450 + i * 30)
      trackGroup.add(track)
    })
    const track = this.drawTrack(size * 1.5, -375)
    trackGroup.add(track)
  ...
  drawGraphicsPromise() {
    ...
    this.fontLoader(this.fontUrl).then(font => {
      const promiseArr = this.data.map((d, i) =>
  ...
  setGraphicSize(value) {
    const s = value / 6,
      max = 100,
      min = 40
    return this.tu.getMiddleValue(max, min, s)
  }
  ...
  drawDecorates() {
    ...
    for (let i = 0; i < this.decorateCount; i++) {
      const radius = this.tu.randomInRange(750, 200)
      const angle = this.tu.randomInRange(-20, 10)
      const x = radius * Math.sin(angle)
      const y = this.randomPos(this.decorateGap)
      const z = radius * Math.cos(angle)

      const scale = this.tu.randomInRange(0.5, 2)
      const m = materials.clone()
      m.color.setStyle(this.tu.randomHex())
  ...
  animation() {
    if (!this.loaded) {
      ...
      this.loaded =
        this.graphics && this.graphics.length === this.data.length && this.tower
  ...
  addLight() {
    ...
    towerLight.shadow.mapSize.width = towerLight.shadow.mapSize.height =
      width * 2.5
  ...
  addHelper() {
    ...
    const ctowerLightHelper = new this.THREE.CameraHelper(
      this.towerLight.shadow.camera
    )
