  ...
  <div class="unit">
    ...
    <el-card>
      <div class="searchBox mb-15">
        ...
        <el-button type="primary" icon="el-icon-plus" @click="createdClick">新建单位</el-button>
      </div>
      ...
      <el-dialog :title="isCreated ? '创建单位' : '修改'" :visible.sync="dialogVisible" :before-close="handleClose" :close-on-click-modal="false" :close-on-press-escape="false" v-draggable="{trigger: '.el-dialog__header', body: '.el-dialog', recover: false}">
...
<script>
import { listAll, getSelectName, saveUnit, delUnit, editUnit, listUnit } from '@/api/asset/standup/standup.js'
...
export default {
  ...
  methods: {
    ...
    async saveUnit () {
      ...
      const res = await this.$simpleAsyncTo(saveUnit(params))
      if (res.flag) {
        this.listAll()
      }
    },
    async delUnit () {
      ...
      const res = await this.$simpleAsyncTo(delUnit(params))
      if (res.flag) {
        this.listAll()
      }
    },
    async editUnit () {
      ...
      const res = await this.$simpleAsyncTo(editUnit(params))
      if (res.flag) {
        this.listAll()
      }
    },
    ...
    async listUnit () {
      ...
      const res = await this.$simpleAsyncTo(listUnit(params))
      if (res.list) {
