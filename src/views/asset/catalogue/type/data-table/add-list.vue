    ...
    <div class="dialog-body" :style="{maxHeight: `${dialogBodyHeight}px`}">
      ...
      <div class="join-table">
        <el-table :data="tableList" v-loading="!!loading" tooltip-effect="dark" @selection-change="handleSelectionChange">
          ...
          <template slot-scope="scope">
              <OverflowTooltip class="width">{{scope.row.dataType}}</OverflowTooltip>
            </template>
          </el-table-column>
...
import {
  tableListPageNoCatalog,
  tableBatchSaveByCatalog
...
export default {
  ...
  methods: {
    ...
    async getTableList() {
      ...
      const res = await this.$simpleAsyncTo(tableListPageNoCatalog(params), '获取数据失败')
      if (res) {
    ...
    async _saveCatalog() {
      this.loading++
      let params = {
        cube_id: this.multipleSelection.map(item => {
          return item.cube_id
        }),
        catalog_id: this.catalogId
      }
      const res = await this.$simpleAsyncTo(tableBatchSaveByCatalog(params), '获取数据失败')
      if (res) {
        this.$emit('refresh')
        this.$message({
          type: 'success',
