<template>
  <section class="task-process page-wrapper">
    <PageTableLayout :showDag="showDag" @leftClickHange="showDag = false">
      ...
      <template v-slot:right>
        <div v-if="isShow === 'info'" class="detail">
          ...
          <el-table ref="returnFields" :data="baseinfo.returnFields" tooltip-effect="dark" style="width: 100%;" max-height="170" :selectable="checkboxSelect">
              ...
              <el-row class="mb-10">
                <el-col>
                  <el-form-item label="审批说明" prop="message">
                    <el-input type="textarea" :rows="4" resize="none" :show-word-limit="true" :maxlength="128" v-model="approvalInfo.message" disabled>
                    </el-input>
                  </el-form-item>
                </el-col>
              </el-row>
...
import { getListDid } from '@/api/notice/task-center/index.js'
export default {
  name: 'Process',
  mixins: [mixin],
  components: {
  ...
  methods: {
    ...
    async getTableList () {
      ...
      const res = await this.$simpleAsyncTo(getListDid(params), '获取列表失败')
      if (res) {
    ...
    approveHandle (row) {
      ...
      this.getToDoDetail(row.proc_inst_id, 'did')
    }
  }
}
</script>
