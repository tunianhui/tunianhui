        ...
        <el-row>
          <el-col :span="24">
            ...
            <el-form-item label="默认格式" prop="partition_default_format">
              <el-select v-model="ProjectForm.partition_default_format" placeholder="请选择">
                <el-option
...
import {queryDataTypeList,queryBusinessDate, editBizPartiton} from '@/api/plan/data-architecture'
...
export default {
    ...
    return {
      ...
      ProjectFormRules: {
        ...
        partition_default_format: [
          {required: true, message: '请选择默认格式', trigger: 'change'},
        ],
        space_type: [
  ...
  created() {
    ...
    this.getParameterSlect()
    if (JSON.stringify(this.daytData) === '{}') {
      this.ProjectForm = this.resetProjectForm()
      // this.is_disabled = false
      this.title = '新建项目'
    } else {
      this.ProjectForm.partition_code = this.daytData.partition_code
      this.ProjectForm.partition_name = this.daytData.partition_name
      this.ProjectForm.partition_data_type = this.daytData.partition_data_type
      this.ProjectForm.partition_default_value =
        this.daytData.partition_default_value
      this.ProjectForm.partition_desc = this.daytData.partition_desc
      this.ProjectForm.partition_default_format =
        this.daytData.partition_default_format
      // this.ProjectForm.space_type = this.daytData.space_type
  ...
  methods: {
    // 获取数据类型
    async getParameterSlect() {
      const res = await this.$simpleAsyncTo(queryBusinessDate())
      if (res) {
    ...
    //       } else {
    //         const res = await this.$simpleAsyncTo(
    //           editProject({...this.ProjectForm, project_id: this.daytData.id}),
    //           '项目信息设置失败'
    //         )
    //         if (res) {
    //           this.$message({
