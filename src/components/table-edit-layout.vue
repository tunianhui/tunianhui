<template>
  <section class="table-edit-layout">
    <div :class="[
        'tel-head',
      ...
      <div class="tel-head-btns">
        <template v-for="(btn, index) in buttons">
          <el-button type="text"
                     :class="['tel-head-btns-item', btn.class]"
                     :key="(btn.title || btn.label) + index"
                     @click="btnClick(btn)"
                     :title="btn.title"
                     :icon="btn.icon"
...
.table-edit-layout {
  ...
  .tel-head {
    ...
    box-shadow: 0 1px 0 0 rgba($grey5, 0.3);
    &.bc-active {
