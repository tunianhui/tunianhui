<template>
  <section class="g6-flow">
      ...
      <div class="flow-node-info" v-show="visibleNodeInfo">
        <div class="flow-node-info-item">
          ...
          <span class="value">
            ...
            <span
              class="tag"
              v-if="!!nodeInfo.cube_id"
              v-clipboard="{value: nodeInfo.cube_id}"
              >复制</span
            >
          </span>
        </div>
        ...
        <div class="flow-node-info-item" v-if="type !== 'task'">
      ...
      <div class="flow-bottom-tool-left">
        <div class="zoom-input">
          <el-input
            v-model.number="zoom"
            @blur="setZoom"
            @keyup.enter.native.stop="setZoom"
          ></el-input
          >&nbsp;%
        </div>
    ...
    <div class="flow-tooltip" ref="tooltip" v-show="visibleTooltip">
      {{ tooltipContent }}
    </div>
    <Contextmenu
      ref="contextmenu"
...
export default {
  ...
  methods: {
    ...
    initGraph(data) {
      ...
      // edge polyline的offset 需要与ranksep配合使用，否则折线有圆角时线会有冒出
    ...
    rerenderGraph() {
      if (!this.graphData || !this.graph) return
      this.graph.read(this.graphData)
    ...
    centerGraph() {
      ...
      const offsetHeight =
        initialTranslate * (containerHeight - height < initialTranslate ? 0 : 1)
      this.graph.moveTo(
        (containerWidth - width - initialTranslate + nodeWidth) / 2,
        (containerHeight - height - initialTranslate + nodeHeight) / 2 -
          offsetHeight
      )
    },
    bindEvent() {
    ...
    showContextmenu(ev) {
      ...
      const {width, height, x: left, y: top} = ev.item.getBBox(),
        ...
        {x: graphLeft, y: graphTop} = this.$refs.flow.getBoundingClientRect()

      let scale = this.graph.getZoom(),
        x = left * scale + graphLeft + canvasTranslateX,
        y = top * scale + graphTop + canvasTranslateY
      if (this.nodeInfo.runStatus) {
        this.changeContextmenu(this.nodeInfo.runStatus === 'pause')
      }
    ...
    changeContextmenu(flag) {
      const menu = flag ? pause : recover
      this.contextmenu.splice(this.contextmenu.length - 1, 1, menu)
    },
    triggerTooltip(ev) {
